...
@section('content')
<div class="page-header"><h1 class="page-title">Ledger Pergerakan Stok Harian</h1></div>
<div class="card mb-3"><div class="card-body py-2">
    <form method="GET" class="row g-2 align-items-center">
        ...
        <div class="col-md-2"><select name="ingredient_id" class="form-select"><option value="">Semua Bahan</option>@foreach($ingredients as $i)<option value="{{ $i->id }}" {{ request('ingredient_id')==$i->id?'selected':'' }}>{{ $i->name }}</option>@endforeach</select></div>
        <div class="col-md-2"><select name="movement_type" class="form-select"><option value="">Semua Tipe</option><option value="purchase_in">Purchase In</option><option value="transfer_in">Transfer In</option><option value="transfer_out">Transfer Out</option><option value="production_in">Production In</option><option value="production_out">Production Out</option><option value="waste">Waste</option><option value="opname_adjustment">Opname Adj.</option></select></div>
        ...
        <div class="col-md-auto ms-auto d-flex gap-2"><button type="submit" class="btn btn-primary">Cari</button><a href="{{ route('inventory.ledger.index') }}" class="btn btn-outline-secondary">Reset</a></div>
    </form>
</div></div>
